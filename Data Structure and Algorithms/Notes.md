...
- **_Constructors_**
  ...
  **"_this_" keyword is used to differentiate between the variables. _this_ keyword stored the address of the current class object (can be used with value at (->) operator)**
  - Copy constructors
...
Linked Lists are linear data structures which store some data and memory location leading to other linked lists.  
Linked Lists are of the following types:

- Singly Linked List

  Lists which have one field for data and other field storing the address of the next Linked List are called Singly Linked Lists. These have one _head_ (Starting node of a Linked List) and maybe a _tail_(Ending node of a Linked List).  
  Linked Lists are preferred when memory usage needs to be optimized. Since, LL nodes are spread throughout the memory unlike arrays or vectors which consume continous memory.

  **Advantages of Linked Lists**
  - Memory Optimization
  - Dynamic in nature (can grow or shrink according to need)
  - No shift while insertion or deletion of elements from any point
  ...
  (refer to "singlyLL.cpp" inside "18. Linked Lists & Problems")

- Doubly Linked List

  Linked Lists whose nodes have 3 fields,
  - Data Field
  - Next Node Address
  - Previous Node Address

  are referred to as Doubly Linked Lists.

  **Advantages over Singly Linked Lists**
  - Traversal can be done both in forward and reverse directions
  - From each node, both it's predecessor and successor nodes can be accessed
  ...
  ```cpp
    class doublyLinkedList
    {
      public:
        int dataField;
        doublyLinkedList *nextList;
        doublyLinkedList *previousList;
    };
  ```

  (refer to "doublyLL.cpp" inside "18. Linked Lists & Problems")

- Circular Linked List

  Lists having no head, only a tail which points to the first element of the list forming a circular structure.  
  Circlular Linked Lists can be of both singly and doubly types.

  **Advantages**
  - Only one (tail) pointer is needed to perform different operations
  - Less number of base cases
  ...
  ```cpp
    class singlyCircularLinkedList
    {
      public:
        int dataField;
        singlyCircularLinkedList *nextList
    };
    ...
    class doublyCircularLinkedList
    {
      public:
        int dataField;
        doublyCircularLinkedList *nextList;
        doublyCircularLinkedList *previousList;
    };
  ```
